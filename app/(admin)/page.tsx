...
export default async function Dashboard() {
  ...
  const cards = [
    ...
    { label: "Code Requests",      value: stats.requests,     color: "text-red-400" },
        ...
        {cards.map(card => (
          <div key={card.label} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-6">
            ...
            <p className={`text-4xl font-black ${card.color}`}>
              {card.value === null ? <span className="text-red-400 text-lg">error</span> : card.value}
            </p>
          </div>
        ))}
      </div>
